...
class MainViewModel(application: Application): AndroidViewModel(application) {
    ...
    private val alignmentDao = AlignmentDatabase.getInstance(application.applicationContext).alignmentDao()
    private val classDao = ClassDatabase.getInstance(application.applicationContext).classDao()
    private val conditionDao = ConditionDatabase.getInstance(application.applicationContext).conditionDao()
    private val damageTypeDao = DamageTypeDatabase.getInstance(application.applicationContext).damageTypeDao()
    private val equipmentCategoryDao = EquipmentCategoryDatabase.getInstance(application.applicationContext).equipmentCategoryDao()
    private val equipmentDao = EquipmentDatabase.getInstance(application.applicationContext).equipmentDao()
    private val featDao = FeatDatabase.getInstance(application.applicationContext).featDao()
    private val featureDao = FeatureDatabase.getInstance(application.applicationContext).featureDao()
    private val languageDao = LanguageDatabase.getInstance(application.applicationContext).languageDao()
    private val magicItemDao = MagicItemDatabase.getInstance(application.applicationContext).magicItemDao()
    private val magicSchoolDao = MagicSchoolDatabase.getInstance(application.applicationContext).magicSchoolDao()
    private val monsterDao = MonsterDatabase.getInstance(application.applicationContext).monsterDao()
    private val proficiencyDao = ProficiencyDatabase.getInstance(application.applicationContext).proficiencyDao()
    private val raceDao = RaceDatabase.getInstance(application.applicationContext).raceDao()
    private val ruleDao = RuleDatabase.getInstance(application.applicationContext).ruleDao()
    private val ruleSectionDao = RuleSectionDatabase.getInstance(application.applicationContext).ruleSectionDao()
    private val skillDao = SkillDatabase.getInstance(application.applicationContext).skillDao()
    private val spellDao = SpellDatabase.getInstance(application.applicationContext).spellDao()
    private val subclassDao = SubclassDatabase.getInstance(application.applicationContext).subclassDao()
    private val subraceDao = SubraceDatabase.getInstance(application.applicationContext).subraceDao()
    private val traitDao = TraitDatabase.getInstance(application.applicationContext).traitDao()
    private val weaponPropertyDao = WeaponPropertyDatabase.getInstance(application.applicationContext).weaponPropertyDao()
