...
class MainViewModel(application: Application, private val categoryFetcherUseCase: CategoryFetcherUseCase): AndroidViewModel(application) {
    ...
    private val damageTypeDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).damageTypeDao()
    private val equipmentCategoryDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).equipmentCategoryDao()
    private val equipmentDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).equipmentDao()
    private val featDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).featDao()
    private val featureDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).featureDao()
    private val languageDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).languageDao()
    private val magicItemDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).magicItemDao()
    private val magicSchoolDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).magicSchoolDao()
    private val monsterDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).monsterDao()
    private val proficiencyDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).proficiencyDao()
    private val raceDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).raceDao()
    private val ruleDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).ruleDao()
    private val ruleSectionDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).ruleSectionDao()
    private val skillDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).skillDao()
    private val spellDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).spellDao()
    private val subclassDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).subclassDao()
    private val subraceDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).subraceDao()
    private val traitDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).traitDao()
    private val weaponPropertyDao = MainDatabase.getInstance(getApplication<Application>().applicationContext).weaponPropertyDao()
