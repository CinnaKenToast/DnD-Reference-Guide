...
class MainViewModel(application: Application): AndroidViewModel(application) {
    ...
    private val alignmentDao = MainDatabase.getInstance(application.applicationContext).alignmentDao()
    private val classDao = MainDatabase.getInstance(application.applicationContext).classDao()
    private val conditionDao = MainDatabase.getInstance(application.applicationContext).conditionDao()
    private val damageTypeDao = MainDatabase.getInstance(application.applicationContext).damageTypeDao()
    private val equipmentCategoryDao = MainDatabase.getInstance(application.applicationContext).equipmentCategoryDao()
    private val equipmentDao = MainDatabase.getInstance(application.applicationContext).equipmentDao()
    private val featDao = MainDatabase.getInstance(application.applicationContext).featDao()
    private val featureDao = MainDatabase.getInstance(application.applicationContext).featureDao()
    private val languageDao = MainDatabase.getInstance(application.applicationContext).languageDao()
    private val magicItemDao = MainDatabase.getInstance(application.applicationContext).magicItemDao()
    private val magicSchoolDao = MainDatabase.getInstance(application.applicationContext).magicSchoolDao()
    private val monsterDao = MainDatabase.getInstance(application.applicationContext).monsterDao()
    private val proficiencyDao = MainDatabase.getInstance(application.applicationContext).proficiencyDao()
    private val raceDao = MainDatabase.getInstance(application.applicationContext).raceDao()
    private val ruleDao = MainDatabase.getInstance(application.applicationContext).ruleDao()
    private val ruleSectionDao = MainDatabase.getInstance(application.applicationContext).ruleSectionDao()
    private val skillDao = MainDatabase.getInstance(application.applicationContext).skillDao()
    private val spellDao = MainDatabase.getInstance(application.applicationContext).spellDao()
    private val subclassDao = MainDatabase.getInstance(application.applicationContext).subclassDao()
    private val subraceDao = MainDatabase.getInstance(application.applicationContext).subraceDao()
    private val traitDao = MainDatabase.getInstance(application.applicationContext).traitDao()
    private val weaponPropertyDao = MainDatabase.getInstance(application.applicationContext).weaponPropertyDao()
