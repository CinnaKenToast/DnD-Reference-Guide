...
@Composable
fun ClassCardList(classList: List<ClassType>, navController: NavController) {
    LazyColumn(
        verticalArrangement = Arrangement.spacedBy(8.dp),
        contentPadding = PaddingValues(8.dp)
    ) {
        items(items = classList) { classType ->
            ClassCard(classType, navController)
        }
...
@Composable
fun ClassCard(classType: ClassType, navController: NavController) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .wrapContentHeight()
            .clickable { navController.navigate("classPage/${classType.index}") }
    ) {
        Column(
...
@Composable
fun ClassPage(classType: ClassType){
    Box(
